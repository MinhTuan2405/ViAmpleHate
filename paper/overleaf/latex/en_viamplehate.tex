...
\usepackage{array}
\usepackage{float}
\usepackage{flafter} % floats never appear before their definition point
\usepackage{tikz}
...
Automatic hate speech detection has evolved from lexicon- and feature-based classifiers \citep{davidson2017,schmidt2017survey} to deep sequence models and, more recently, pretrained transformers. A persistent difficulty is \emph{implicit} hate: messages that convey hostility without explicit slurs, relying on sarcasm, stereotypes, coded references, or in-group knowledge \citep{elsherief2021latent}. Implicit hate is often more common than overt slurs and substantially harder for keyword-driven systems, motivating approaches that reason about \emph{who} is targeted and \emph{how}.
...
PhoBERT \citep{phobert} is a RoBERTa-style \citep{roberta} model---itself a robustly optimized variant of BERT \citep{bert}, which builds on the Transformer architecture \citep{transformer}---pretrained on large Vietnamese corpora and is the de facto encoder for Vietnamese text classification. Because PhoBERT is trained on \emph{word-segmented} text, word segmentation (e.g., with VnCoreNLP \citep{vncorenlp}) precedes tokenization. Hybrid architectures such as PhoBERT-CNN \citep{to2022phobertcnn} add local feature extractors on top of contextual embeddings. We use PhoBERT-base as the shared encoder for both baseline and proposed models so that differences are attributable to target-aware modeling rather than the backbone.
...
\begin{figure*}[!t]
  ...
  \begin{tikzpicture}[
    ...
    \node[base]  (pre) at (2.8,0)    {Preprocessing\\{\scriptsize normalize $\cdot$ word-seg $\cdot$ tokenize}};
    \node[base]  (enc) at (5.8,1.9)  {PhoBERT encoder\\{\scriptsize $H=[h_0,\dots,h_n]$,\ $h_0={}$[CLS]}};
    \node[novel] (det) at (5.8,-1.9) {Multi-signal extraction\\{\scriptsize Vi-NER $+$ target cues $\to T_x$}\\{\scriptsize attack cues $\to A_x$; fallback [CLS]}};
  ...
  \caption{Overall architecture of ViAmpleHate. The comment is normalized, word-segmented, and encoded by PhoBERT into hidden states $H=[h_0,\dots,h_n]$, with $h_0$ the [CLS] (global) representation. Vietnamese NER and a target-cue bank produce the target index set $T_x$, while an attack-cue bank produces $A_x$ (falling back to [CLS] when a set is empty). A relation-bank attention with three per-sample channels---explicit target ($r_{\mathrm{exp}}$), implicit context ($r_{\mathrm{imp}}$), and attack ($r_{\mathrm{atk}}$)---is fused into a single relation vector $r$ and injected into $h_0$ through an instance-adaptive gate ($g=\sigma(W_g[h_0;r])$, $z=h_0+g\,r$). A linear classifier with a validation-tuned threshold yields the NON-HATE/HATE decision; training minimizes $L=L_{\mathrm{CE}}+\alpha L_{\mathrm{CL}}$.}
  \label{fig:arch}
\end{figure*}
...
\begin{aligned}
  ...
  T_x &\leftarrow \{0\}\ \text{if } T_x=\varnothing, \\
  A_x &\leftarrow \{0\}\ \text{if } A_x=\varnothing.
\end{aligned}
\end{equation}
...
\end{equation}
with padding masked before softmax ($\mathrm{scores}_j=-\infty$ if $\mathrm{mask}_j=0$). We treat this as a correction to our own port rather than a claim about the original AmpleHate, and do not evaluate it in isolation.
...
We train with $L = L_{\mathrm{CE}} + \alpha\,L_{\mathrm{CL}}$, $\alpha=0.1$. Weighted cross-entropy $L_{\mathrm{CE}}=-\sum_c w_c y_c\log\hat y_c$ handles imbalance by up-weighting HATE, with label smoothing \citep{muller2019labelsmoothing} (the class weights $w_c$, the label-smoothing value, and the margin $m$ below are reported in Appendix~\ref{sec:hparams}). The contrastive term on $z$ (with $s_{ij}=\cos(z_i,z_j)$) pulls same-class pairs together and pushes different-class pairs apart:
\begin{equation}
\begin{split}
...
In terms of training dynamics, Figure~\ref{fig:curves} shows that ViAmpleHate converges stably on ViHSD, reaching its best validation macro-F1 at epoch 4.

% FIGURE 3 — Training curves: training_curves_viamplehate.png
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{image/figure3.png}
  \caption{Training/validation curves of ViAmpleHate on ViHSD (best epoch 4, val macro-F1 $=0.7852$). \texttt{training\_curves\_viamplehate.png}.}
  \label{fig:curves}
\end{figure}
...
Each model is reported at its own intended configuration (Section~\ref{sec:reshape}, Table~\ref{tab:arch}); the proposed-vs-baseline rows therefore compare the full proposed system against the baseline as configured.
...
\begin{table}[!t]
  ...
  \caption{Results on ViHSD (test). Best per column in bold (accuracy shown for reference).}
  \label{tab:vihsd}
\end{table}
...
\begin{table}[!t]
  ...
  \caption{Results on VOZ-HSD (test). Best per column in bold among models (accuracy shown for reference; the baseline has the highest accuracy).}
  \label{tab:voz}
\end{table}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{lcccccccc}
    ...
    \textbf{Model} & P & R & F1 & P & R & F1 & \textbf{Macro-F1} & Acc. \\
    \midrule
...
\begin{table}[!t]
  ...
  \caption{Per-class results of ViAmpleHate (test set).}
  \label{tab:perclass}
\end{table}

At the aggregate level, the confusion matrices in Figure~\ref{fig:cm} show that ViAmpleHate reduces HATE false positives relative to the baseline on ViHSD.

% FIGURE 4 — Confusion matrices ViHSD: baseline vs proposed.
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.45\linewidth]{image/figure4a.png}\hfill
  \includegraphics[width=0.45\linewidth]{image/figure4b.png}
  \caption{Confusion matrices on ViHSD: baseline AmpleHate (left) vs ViAmpleHate (right). \texttt{confusion\_matrix\_amplehate.png} and \texttt{confusion\_matrix\_viamplehate.png}; note the reduction in HATE false positives.}
  \label{fig:cm}
\end{figure*}
...
From a deployment standpoint, the per-class picture in Table~\ref{tab:detail} matters more than a single headline number. A moderation system that wants to minimize wrongful HATE flags should weigh HATE precision heavily: there, ViAmpleHate is the strongest transformer on VOZ-HSD (0.73) and is competitive on ViHSD (0.62), while the recall-heavy BiLSTM baseline would generate far more false accusations. A system that instead wants to surface as much potential hate as possible for human review might tolerate the lower-precision, higher-recall regime of the lexical baselines, but in that case the validation-tuned threshold (Appendix~\ref{sec:hparams}) can simply be lowered, trading precision for recall along the same model. The added components are also cheap relative to the PhoBERT backbone---three lightweight attention heads, a gate, and a one-pass cue match at preprocessing time---so the gains do not come at a meaningful inference-cost premium, and the corrected batched attention is in fact cheaper than the spurious $B\times B$ formulation it replaces.
