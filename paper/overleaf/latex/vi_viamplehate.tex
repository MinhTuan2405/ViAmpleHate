...
\usepackage{array}
\usepackage{float}
\usepackage{flafter} % float không bao giờ xuất hiện trước vị trí khai báo
\usepackage{tikz}
...
Mạng xã hội tiếng Việt phát triển nhanh, và đi cùng với đó là sự lan rộng của nội dung thù ghét, lăng mạ. Những nội dung này gây tổn thương cho cá nhân và cộng đồng bị nhắm tới, đồng thời đặt các nền tảng trước rủi ro về pháp lý lẫn uy tín. Khi lượng nội dung do người dùng tạo ra vượt xa khả năng kiểm duyệt thủ công, phát hiện phát ngôn thù ghét tự động gần như trở thành một nhu cầu bắt buộc. Tuy vậy, phần lớn tiến bộ cho đến nay vẫn dồn về tiếng Anh và các ngôn ngữ giàu tài nguyên, nên khi chuyển sang tiếng Việt, hiệu quả thường sụt giảm đáng kể.
...
\end{enumerate}
Thông điệp rộng hơn là: target-aware vốn là một inductive bias hữu ích cho bài toán phát hiện phát ngôn thù ghét, nhưng chỉ thực sự phát huy khi được thích ứng với đặc trưng bề mặt của ngôn ngữ đích, thay vì bê nguyên xi từ tiếng Anh sang.

\section{Công trình liên quan}

\subsection{Phát hiện phát ngôn thù ghét và thù ghét hàm ẩn}
Các phương pháp phát hiện phát ngôn thù ghét tự động đã phát triển từ classifier dựa trên từ điển và đặc trưng thủ công \citep{davidson2017,schmidt2017survey}, qua các mô hình chuỗi sâu, đến các transformer tiền huấn luyện gần đây. Một trở ngại dai dẳng là thù ghét \emph{hàm ẩn}: thông điệp thể hiện sự thù địch mà không dùng slur tường minh, thay vào đó dựa vào mỉa mai, định kiến, tham chiếu được mã hóa hoặc kiến thức nội nhóm \citep{elsherief2021latent}. Dạng này thường gặp hơn slur lộ liễu và đặc biệt khó đối với các hệ thống dựa trên từ khóa, qua đó thúc đẩy những hướng tiếp cận có khả năng suy luận \emph{ai} bị nhắm tới và \emph{bị nhắm tới theo cách nào}.

\subsection{AmpleHate}
AmpleHate \citep{amplehate} xử lý thù ghét hàm ẩn bằng cách khuếch đại attention giữa biểu diễn toàn cục của câu và các target ứng viên. Mô hình trích target token bằng NER, tính một HeadAttention vector với query từ \texttt{[CLS]} và key/value từ target token, rồi inject kết quả này với một hệ số cố định trước khi phân loại. Ý tưởng cốt lõi---rằng target là tín hiệu then chốt---chính là nguồn cảm hứng trực tiếp cho công trình này. Dù vậy, những hạn chế khi đem AmpleHate sang tiếng Việt cũng hiện ra khá rõ: mô hình mặc định NER và phạm trù target kiểu tiếng Anh, inject thông tin quan hệ với cùng một cường độ cho mọi mẫu, và chỉ mô hình hóa duy nhất quan hệ target mà bỏ qua vị từ thù địch (\emph{attack})---chính là yếu tố biến một lời nhắc tên thành lời thù ghét.
...
PhoBERT \citep{phobert} là mô hình kiểu RoBERTa \citep{roberta}---vốn là một biến thể được tối ưu mạnh của BERT \citep{bert}, dựa trên kiến trúc Transformer \citep{transformer}---được tiền huấn luyện trên kho ngữ liệu tiếng Việt lớn, hiện thường được dùng làm encoder mặc định cho phân loại văn bản tiếng Việt. Vì PhoBERT được huấn luyện trên văn bản đã được \emph{tách từ} (word segmentation), bước tách từ (ví dụ bằng VnCoreNLP \citep{vncorenlp}) cần được thực hiện trước khi token hóa. Các kiến trúc lai như PhoBERT-CNN \citep{to2022phobertcnn} gắn thêm bộ trích đặc trưng cục bộ lên trên embedding ngữ cảnh. Nhóm dùng PhoBERT-base làm encoder chung cho cả baseline lẫn mô hình đề xuất, để khác biệt về kết quả chủ yếu phản ánh cách mô hình hóa target-aware thay vì khác biệt ở backbone.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{image/figure1.png}
  \caption{Phân phối lớp của ViHSD và VOZ-HSD sau khi gán nhãn nhị phân.}
  \label{fig:dist}
\end{figure}
...
\begin{equation}
\begin{aligned}
  T_x &= M_{\mathrm{NER}}(x)\cup M_{\mathrm{target}}(x), \\
  A_x &= M_{\mathrm{attack}}(x), \\
  T_x &\leftarrow \{0\}\ \text{nếu } T_x=\varnothing, \\
  A_x &\leftarrow \{0\}\ \text{nếu } A_x=\varnothing.
\end{aligned}
\end{equation}
...
\end{equation}
trong đó phần padding được mask trước softmax ($\mathrm{scores}_j=-\infty$ nếu $\mathrm{mask}_j=0$). Nhóm xem đây là một chỉnh sửa cho bản port của mình, không phải một khẳng định về AmpleHate gốc, nên không tách phần này ra đánh giá như một thí nghiệm độc lập.
...
Về động học huấn luyện, Hình~\ref{fig:curves} cho thấy ViAmpleHate trên ViHSD hội tụ ổn định và đạt macro-F1 validation tốt nhất ở epoch 4.
...
Bên cạnh điểm số cuối cùng, nhóm còn muốn biết liệu mỗi thay đổi có thực sự khắc phục một hạn chế cụ thể của baseline hay không---từ độ phủ thấp, việc chưa mô hình hóa sự thù địch, cho đến cách hợp nhất tín hiệu và hàm mất mát. Riêng độ phủ target được khảo sát trực tiếp ở Mục~\ref{sec:coverage}.

\section{Kết quả và phân tích lỗi}

\subsection{Kết quả chính}

Mỗi mô hình được báo cáo ở đúng cấu hình dự kiến của nó (Mục~\ref{sec:reshape}, Bảng~\ref{tab:arch}), nên các dòng đề xuất so với baseline phản ánh phép so sánh giữa toàn bộ hệ thống đề xuất và baseline ở cấu hình tương ứng.
...
\begin{table}[!t]
  ...
  \caption{Kết quả trên tập test ViHSD. AmpleHate là baseline, $\Delta$ là chênh lệch của ViAmpleHate. Giá trị tốt nhất mỗi cột in đậm (accuracy chỉ để tham khảo).}
  \label{tab:vihsd}
\end{table}
...
\begin{table}[!t]
  ...
  \caption{Kết quả trên tập test VOZ-HSD. Giá trị tốt nhất mỗi cột in đậm (accuracy chỉ để tham khảo; baseline có accuracy cao nhất).}
  \label{tab:voz}
\end{table}

Ở những độ đo quan trọng, các mô hình transformer vượt trội rõ rệt so với baseline từ vựng và embedding tĩnh (Bảng~\ref{tab:vihsd} và~\ref{tab:voz}). Điều này cho thấy biểu diễn ngữ cảnh là thật sự cần thiết một khi thù ghét phụ thuộc vào context, vào cách diễn đạt phi chính thức và vào tương tác giữa lời nhắc tới target với vị từ thù địch. Trong nhóm transformer, baseline AmpleHate nhỉnh hơn một PhoBERT classifier thuần nhờ target-aware attention, nhưng lợi ích này bị giới hạn vì NER tiếng Anh hiếm khi tìm được target tiếng Việt hợp lệ, khiến hầu hết đầu vào quay về \texttt{[CLS]}. Nhóm chỉ so sánh với các mô hình tự huấn luyện theo cùng một giao thức; việc đối chiếu với kết quả ViHSD đã công bố được dành cho công trình tương lai.

Trên VOZ-HSD, ViAmpleHate cải thiện cả macro-F1 lẫn HATE-F1 với biên độ rõ ràng (HATE-F1 $+0{,}0508$). Trên ViHSD, khác biệt nhỏ hơn nhiều ($+0{,}0027$ macro-F1, $+0{,}0036$ HATE-F1). Vì chưa có kiểm định ý nghĩa thống kê, nhóm tạm xem mức tăng trên ViHSD là kết quả sơ bộ. Đáng lưu ý, accuracy lại \emph{giảm} trên VOZ-HSD trong khi macro-F1 và HATE-F1 đều tăng; đây là minh chứng cho thấy accuracy dễ gây hiểu nhầm khi dữ liệu mất cân bằng.

\subsection{Kết quả chi tiết theo từng lớp}
\label{sec:detailed}
\begin{table*}[!t]
  \centering
  \small
  ...
  \begin{tabular}{lcccccccc}
    \toprule
    & \multicolumn{3}{c}{\textbf{NON-HATE}} & \multicolumn{3}{c}{\textbf{HATE}} & & \\
    \cmidrule(lr){2-4}\cmidrule(lr){5-7}
    \textbf{Mô hình} & P & R & F1 & P & R & F1 & \textbf{Macro-F1} & Acc. \\
    \midrule
    \multicolumn{9}{@{}l}{\textit{(a) ViHSD (test)}} \\
    \midrule
    ...
    \textbf{ViAmpleHate} & \textbf{0.95} & \textbf{0.96} & \textbf{0.9558} & \textbf{0.62} & \textbf{0.60} & \textbf{0.6081} & \textbf{0.7819} & \textbf{0.9205} \\
    \midrule
    \multicolumn{9}{@{}l}{\textit{(b) VOZ-HSD (test)}} \\
    \midrule
    ...
    \textbf{ViAmpleHate} & \textbf{0.96} & \textbf{0.97} & \textbf{0.9678} & \textbf{0.73} & \textbf{0.68} & \textbf{0.7065} & \textbf{0.8371} & \textbf{0.9420} \\
    \bottomrule
  \end{tabular}
  \caption{Báo cáo phân loại chi tiết theo từng lớp trên ViHSD (a) và VOZ-HSD (b), tập test. Macro-F1 là độ đo chính; dòng ViAmpleHate in đậm để làm nổi bật (không phải dấu hiệu tốt nhất theo cột). Precision/recall theo lớp lấy hai chữ số thập phân, còn F1 và macro-F1 lấy bốn chữ số.}
  \label{tab:detail}
\end{table*}

Bảng~\ref{tab:detail} trình bày báo cáo phân loại đầy đủ theo từng lớp cho toàn bộ mô hình trên hai tập test, lấy macro-F1 làm độ đo chính. Việc tách riêng từng lớp giúp thấy rõ ảnh hưởng của mất cân bằng: một mô hình có thể đạt F1 của NON-HATE rất cao (trên 0,90 với mọi mô hình transformer) trong khi F1 của HATE---cột thực sự đáng quan tâm---trải trên một khoảng rộng hơn nhiều, từ 0,4187 đến 0,7065. Dòng của mô hình đề xuất được in đậm để làm nổi bật; mô hình này đạt macro-F1 tốt nhất trên cả hai bộ dữ liệu (0,7819 trên ViHSD và 0,8371 trên VOZ-HSD).

Báo cáo chi tiết cho thấy mức tăng macro-F1 đến từ đâu. Trên VOZ-HSD, mô hình đề xuất nâng precision của HATE từ 0,65 lên 0,73 trong khi recall của HATE gần như giữ nguyên so với baseline (0,68 so với 0,66); nhờ vậy, mức tăng $+0{,}0508$ ở HATE-F1 chủ yếu đến từ việc dự đoán dương sạch hơn, chứ không phải do gắn nhãn HATE cho nhiều bình luận hơn. Hai baseline từ vựng và embedding tĩnh nằm ở hai thái cực của đánh đổi precision--recall: BiLSTM + fastText đạt recall HATE cao nhất trên cả hai bộ dữ liệu (0,77 trên ViHSD và 0,91 trên VOZ-HSD) nhưng precision HATE lại thấp đến mức (0,38 và 0,27) khiến macro-F1 của nó yếu nhất trong sáu mô hình; ngược lại, TF-IDF + SVM rất dè dặt---precision HATE cao, recall HATE thấp---nên bỏ sót phần lớn trường hợp thù ghét. Các mô hình transformer bám sát nhau hơn nhiều, và trong nhóm này, góc nhìn theo lớp cho thấy lợi thế của ViAmpleHate tập trung ở cột HATE, còn F1 của NON-HATE gần như ngang bằng baseline---đúng với hành vi ta mong muốn khi lớp thiểu số mới là lớp cần quan tâm.
...
\begin{table}[!t]
  ...
  \caption{Kết quả theo từng lớp của ViAmpleHate (tập test).}
  \label{tab:perclass}
\end{table}

Ở cấp độ tổng thể, ma trận nhầm lẫn trong Hình~\ref{fig:cm} cho thấy ViAmpleHate giảm số false positive của lớp HATE so với baseline trên ViHSD.

% HÌNH 4 — Ma trận nhầm lẫn ViHSD: baseline so với đề xuất.
\begin{figure*}[!t]
  \centering
...
Xét từ góc độ triển khai, bức tranh theo từng lớp trong Bảng~\ref{tab:detail} có ý nghĩa hơn một con số tổng hợp đơn lẻ. Một hệ thống kiểm duyệt muốn hạn chế việc gắn nhãn HATE oan nên ưu tiên precision của HATE: ở tiêu chí này, ViAmpleHate là mô hình transformer mạnh nhất trên VOZ-HSD (0,73) và vẫn cạnh tranh trên ViHSD (0,62), trong khi baseline thiên về recall như BiLSTM sẽ tạo ra nhiều cáo buộc nhầm hơn hẳn. Ngược lại, một hệ thống muốn đưa càng nhiều trường hợp khả nghi ra cho người kiểm duyệt xem xét có thể chấp nhận chế độ precision thấp, recall cao của các baseline từ vựng; nhưng khi đó, ta chỉ cần hạ threshold đã tinh chỉnh trên validation (Phụ lục~\ref{sec:hparams}) để đánh đổi precision lấy recall trên chính mô hình đề xuất. Các thành phần thêm vào cũng nhẹ so với backbone PhoBERT---ba attention head gọn nhẹ, một gate, và một lượt khớp cue ở bước tiền xử lý---nên mức cải thiện gần như không kéo theo chi phí inference đáng kể; thậm chí, batched attention sau khi chỉnh sửa còn rẻ hơn cả công thức $B\times B$ giả tạo mà nó thay thế.
