% =====================================================================
% ViAmpleHate — ACL paper (VERSION v3: EXTENDED, ~10 pages)
% Built from markdown/v3/report_en.md. Adds: broader Related Work (5 subsec),
% cue-bank construction, target-coverage analysis, ablation (fill-in), qualitative
% analysis, Discussion, Ethics Statement, and appendices.
...
% If Vietnamese diacritics fail, use lualatex/xelatex or the T5 fontenc line.
% =====================================================================
...
\begin{enumerate}
  ...
  \item \textbf{Implementation and fusion fixes.} We correct a batch-level attention computation that mixes information across samples, and replace AmpleHate's fixed-scalar injection with an \textbf{instance-adaptive gate}.
...
Contrastive objectives encourage same-class representations to be close and different-class representations to be far apart, improving robustness and class separation, particularly under imbalance \citep{supcon}. We adopt a supervised contrastive term on the post-gate representation as an auxiliary objective alongside weighted cross-entropy, to tighten the boundary between the frequent NON-HATE class and the rare HATE class.
...
\subsection{VOZ-HSD}
VOZ-HSD \citep{vozhsd} consists of comments from the VOZ forum. As with ViHSD, the data is split into train/validation/test, and its post-relabeling distribution is in Table~\ref{tab:data}.
...
\begin{figure*}[t]
  ...
  \caption{Overall architecture of ViAmpleHate. \textbf{[DRAW NEW]} A comment is normalized, word-segmented, and encoded by PhoBERT; Vietnamese NER + target cues yield $T_x$ and attack cues yield $A_x$; three relation channels are fused and injected into $h_0$ through an instance-adaptive gate before classification ($L=L_{\mathrm{CE}}+\alpha L_{\mathrm{CL}}$).}
  \label{fig:arch}
\end{figure*}
...
The original AmpleHate-style implementation computes attention via a multiplication equivalent to $QK^\top$ over the whole batch, so for $Q,K\in\mathbb{R}^{B\times d}$ we get $QK^\top\in\mathbb{R}^{B\times B}$, inadvertently mixing information across samples. ViAmpleHate uses batched attention so each sample attends only to its own tokens, with $Q\in\mathbb{R}^{B\times 1\times d}$, $K\in\mathbb{R}^{B\times m\times d}$:
\begin{equation}
  \mathrm{scores} = \mathrm{bmm}(Q,K^\top)/\sqrt d \in\mathbb{R}^{B\times1\times m},
\end{equation}
with padding masked before softmax ($\mathrm{scores}_j=-\infty$ if $\mathrm{mask}_j=0$).
...
We train with $L = L_{\mathrm{CE}} + \alpha\,L_{\mathrm{CL}}$, $\alpha=0.1$. Weighted cross-entropy $L_{\mathrm{CE}}=-\sum_c w_c y_c\log\hat y_c$ handles imbalance by up-weighting HATE, with label smoothing. The contrastive term on $z$ (with $s_{ij}=\cos(z_i,z_j)$) pulls same-class pairs together and pushes different-class pairs apart:
\begin{equation}
\begin{split}
...
We compare against five baselines, all binary. \textbf{TF-IDF + LR} and \textbf{TF-IDF + SVM} use sparse lexical features. \textbf{BiLSTM + fastText} \citep{fasttext} combines static embeddings with a recurrent model. \textbf{PhoBERT-CNN} stacks CNN feature extractors on PhoBERT. \textbf{AmpleHate-PhoBERT} is a faithful port of the original with English NER, a single HeadAttention, and fixed injection $z=h_0+e\,r_{\mathrm{base}}$ ($e=1.0$); sharing the PhoBERT-base encoder, it is the direct competitor that isolates our changes. Table~\ref{tab:arch} summarizes the differences.
...
Transformer-based models clearly outperform the lexical and static-embedding baselines on the metrics that matter (Tables~\ref{tab:vihsd},~\ref{tab:voz}), confirming that contextual representations are important when hate depends on context, informal phrasing, and the interaction between a mention and a hostile predicate. Within the transformer family, the AmpleHate baseline improves over a plain PhoBERT classifier through target-aware attention, but its benefit is capped because English NER rarely detects valid Vietnamese targets, so most inputs revert to \texttt{[CLS]}.

ViAmpleHate improves macro-F1 and HATE-F1 on both datasets. Gains are modest but consistent on ViHSD and substantially larger on VOZ-HSD, where HATE-F1 rises by $0.0508$. Notably, accuracy \emph{drops} on VOZ-HSD while macro-F1 and HATE-F1 both rise---a direct demonstration that accuracy is the wrong headline metric under imbalance, since the model reallocates capacity from the easy majority class toward the hard minority class.

\subsection{Effect of Vietnamese target extraction}
\label{sec:coverage}
The headline mechanism behind these gains is target coverage. Under English NER, only $\approx$0.09\% of training comments contain a detected target, so the baseline's target-aware pathway is almost never active. Replacing English NER with Vietnamese NER and adding the target-cue bank raises coverage to $\approx$18.8--20.0\% of checked comments---more than two orders of magnitude---which is what gives relation-bank attention something to attend to. This also explains the dataset gap: where cues fire more often and more accurately, the relation channels contribute more and the improvement grows; where coverage is low, the model leans on the implicit \texttt{[CLS]} channel and the margin shrinks.
...
\label{sec:ablation}
To isolate each component, we ablate one at a time from the full model and re-train under otherwise identical settings, reporting macro-F1 and HATE-F1 on the test set. Table~\ref{tab:abl} provides the template; values are to be filled from controlled ablation runs.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    \toprule
    \textbf{Configuration} & \textbf{Mac-F1} & \textbf{HATE-F1} \\
    \midrule
    Full ViAmpleHate                 & 0.7819 & 0.6081 \\
    $-$ attack channel               & [TODO] & [TODO] \\
    $-$ adaptive gate (fixed inj.)   & [TODO] & [TODO] \\
    $-$ contrastive loss (CE only)   & [TODO] & [TODO] \\
    $-$ target cues (NER only)       & [TODO] & [TODO] \\
    $-$ corrected batched attention  & [TODO] & [TODO] \\
    \bottomrule
  \end{tabular}
  \caption{Ablation on ViHSD (test). \textbf{[TODO: fill from ablation runs.]}}
  \label{tab:abl}
\end{table}

% FIGURE 4 (optional) — Gate value distribution. DRAW NEW if gate values are logged.
...
Three observations stand out. First, the improvement is concentrated in the minority class: on ViHSD, HATE precision rises ($0.5972\!\to\!0.6177$) while recall falls slightly ($0.6119\!\to\!0.5988$), so the model becomes more conservative but more precise---desirable when false accusations of hate are costly. Second, the larger VOZ-HSD gains alongside an accuracy drop indicate that the model trades majority-class accuracy for minority-class quality, exactly the trade-off practitioners want when the minority class is the target of interest. Third, the dependence on cue coverage suggests an inexpensive path to further gains: expanding and refining the cue banks should yield additional improvement without architectural change.
...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    Vietnamese NER & \texttt{NlpHUST/ner-vietnamese-electra-base} \\
    max\_len & 256 \\
