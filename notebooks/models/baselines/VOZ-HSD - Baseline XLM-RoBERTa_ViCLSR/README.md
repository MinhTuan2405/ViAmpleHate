...
Cấu hình mặc định dùng max length 128. XLM-RoBERTa-base chạy 5 epochs với batch size 8; ViCLSR chạy 2 epochs với batch size 1 do dùng XLM-RoBERTa-Large và tốn thời gian/VRAM hơn đáng kể. Vì training budget khác nhau, kết quả này là so sánh baseline trong giới hạn tài nguyên Kaggle, không phải so sánh compute-matched. Mỗi `metrics.json` lưu toàn bộ cấu hình thực tế trong trường `config`.
