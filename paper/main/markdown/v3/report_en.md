# ViAmpleHate: A Proposed AmpleHate- and PhoBERT-based Approach for Vietnamese Hate Speech Detection (Extended v3)

> Extended (~10-page) version. Compared with v1/v2 it adds: a broader Related Work (5 subsections), a cue-bank construction subsection, a target-coverage analysis, an ablation table (to be filled from your runs), a qualitative analysis with illustrative examples, a dedicated Discussion section, an Ethics Statement, and expanded appendices. Figure-insertion notes use **📌 [INSERT FIGURE]**; placeholders for numbers you still need to run are marked **[TODO]**.

## Abstract

Detecting hate speech on Vietnamese social media is difficult because hateful intent is rarely carried by explicit named entities; instead it hides in informal group references, slang, sarcasm, and indirect insinuation. Target-aware models such as AmpleHate have shown that attending to the relationship between an utterance and the target it addresses helps detect even implicit hate, yet the original AmpleHate pipeline relies on English named-entity recognition (NER) and English-oriented target categories, so it transfers poorly to Vietnamese: on our training data English NER finds a usable target in only about 0.09% of comments, collapsing the model into a plain sentence classifier. We propose **ViAmpleHate**, a Vietnamese adaptation of AmpleHate built on PhoBERT. ViAmpleHate (i) replaces English NER with Vietnamese NER and a hand-curated *target-cue* bank, raising target coverage to roughly 18.8–20.0%; (ii) adds a separate *attack-cue* bank and models three relation channels — explicit target, implicit context, and attack — through a *relation-bank attention*; (iii) corrects a batch-level attention computation that leaks information across samples; and (iv) replaces fixed-scalar injection with an *instance-adaptive gate*, training the whole model with weighted cross-entropy combined with a contrastive objective. On the ViHSD and VOZ-HSD datasets in a binary NON-HATE/HATE setting, ViAmpleHate improves macro-F1 and the minority HATE-class F1 over a faithful AmpleHate baseline and over TF-IDF, BiLSTM, and PhoBERT-CNN baselines, with the largest gains on the minority class (HATE-F1 +0.0508 on VOZ-HSD). Our analysis shows that the benefit grows with cue coverage, confirming that target-aware modeling must be linguistically adapted to Vietnamese rather than transferred verbatim from English.
...
3. **Implementation and fusion fixes.** We correct a batch-level attention computation that inadvertently mixes information across samples, and we replace AmpleHate's fixed-scalar injection with an **instance-adaptive gate** that decides, per comment, how strongly relation evidence should influence the prediction.
...
Across both datasets, ViAmpleHate improves the two metrics that matter most under imbalance — macro-F1 and HATE-class F1 — with the clearest gains on the minority class. Our analysis ties these gains directly to cue coverage: the more reliably target and attack cues are detected, the more the relation-bank attention helps. The broader message is that target-awareness is a genuinely useful inductive bias for hate speech detection, but only when it is adapted to the linguistic surface of the target language instead of being transferred verbatim from English.
...
VOZ-HSD consists of comments collected from the VOZ forum, originating from the `tarudesu/VOZ-HSD` set on HuggingFace. As with ViHSD, the data is split into training, validation, and test partitions, and its post-relabeling class distribution is reported in Table 1.
...
### 4.6 Corrected batched attention

The original AmpleHate-style implementation computes attention via a multiplication equivalent to `QKᵀ` over the whole batch, so that for `Q,K ∈ R^{B×d}` we obtain `QKᵀ ∈ R^{B×B}`, inadvertently mixing information across samples. ViAmpleHate uses batched attention so that each sample attends only to its own tokens, with `Q ∈ R^{B×1×d}` and `K ∈ R^{B×m×d}`:

$$
\text{scores} = \frac{\text{bmm}(Q,K^\top)}{\sqrt d}\in\mathbb{R}^{B\times1\times m},
$$

and padding positions are masked before softmax (`scores_j = -∞` if `mask_j = 0`).
...
We train with weighted cross-entropy combined with a contrastive loss, `L = L_CE + α·L_CL` with `α = 0.1`. Weighted cross-entropy `L_CE = -Σ_c w_c y_c log ŷ_c` handles imbalance by up-weighting the minority HATE class, with label smoothing to curb overconfidence. The contrastive term, applied to the post-gate `z` with cosine similarity `s_ij = cos(z_i, z_j)`, pulls same-class pairs together and pushes different-class pairs apart:
...
We compare ViAmpleHate against five baselines, all in the binary setting. **TF-IDF + Logistic Regression** and **TF-IDF + SVM** use sparse lexical features and provide non-neural reference points. **BiLSTM + fastText** combines static Vietnamese embeddings with a recurrent sequence model. **PhoBERT-CNN** stacks convolutional feature extractors on PhoBERT embeddings. Finally, **AmpleHate-PhoBERT** is a faithful port of the original AmpleHate with English NER, a single HeadAttention module, and fixed injection `z = h_0 + e·r_base` with `e = 1.0`; because it shares the PhoBERT-base encoder with our model, it is the direct competitor and isolates the effect of our proposed changes. Table 2 summarizes the differences.
...
**Table 4 — Results on VOZ-HSD (test set). Best per column in bold.**
...
Transformer-based models clearly outperform the lexical and static-embedding baselines on the metrics that matter, confirming that contextual representations are important for a task in which hate often depends on context, informal phrasing, and the interaction between a mention and a hostile predicate. Within the transformer family, the AmpleHate baseline improves over a plain PhoBERT classifier through target-aware attention, but its benefit is capped because English NER rarely detects valid Vietnamese targets, so most inputs revert to `[CLS]`.

ViAmpleHate improves macro-F1 and HATE-F1 on both datasets. The gains are modest but consistent on ViHSD and substantially larger on VOZ-HSD, where HATE-F1 rises by 0.0508. Notably, accuracy *drops* on VOZ-HSD while macro-F1 and HATE-F1 both rise — a direct demonstration that accuracy is the wrong headline metric under imbalance, since the model is reallocating capacity from the easy majority class toward the hard minority class.

### 6.2 Effect of Vietnamese target extraction

The headline mechanism behind these gains is target coverage. Under English NER, only ~0.09% of training comments contain a detected target, so the baseline's target-aware pathway is almost never active. Replacing English NER with Vietnamese NER and adding the target-cue bank raises coverage to roughly 18.8–20.0% of checked comments — more than two orders of magnitude — which is what gives the relation-bank attention something to attend to. This also explains the dataset gap: where cues fire more often and more accurately, the relation channels contribute more, and the improvement over the baseline grows; where coverage is low, the model leans on the implicit `[CLS]` channel and the margin shrinks.
...
To isolate the contribution of each component, we ablate them one at a time from the full model. The protocol is to remove a single component (e.g., the attack channel, the adaptive gate, or the contrastive term) and re-train under otherwise identical settings, reporting macro-F1 and HATE-F1 on the test set. Table 5 provides the template; values are to be filled from your ablation runs.

**Table 5 — Ablation on ViHSD (test). [TODO: fill from ablation runs.]**

| Configuration | Macro-F1 | HATE-F1 |
|---|---:|---:|
| Full ViAmpleHate | 0.7819 | 0.6081 |
| − attack channel (target+implicit only) | [TODO] | [TODO] |
| − adaptive gate (fixed injection) | [TODO] | [TODO] |
| − contrastive loss (CE only) | [TODO] | [TODO] |
| − target cues (NER only) | [TODO] | [TODO] |
| − corrected batched attention | [TODO] | [TODO] |

> 📌 **[INSERT FIGURE — optional] Figure 4 — Gate value distribution.** Histogram of the learned gate `g` over the test set, split by predicted class. **Draw new** if you log gate values; illustrates that confident HATE predictions tend to use larger `g`.
...
**Table 7 — Per-class results of ViAmpleHate on ViHSD (test).**
...
Three observations stand out. First, the improvement is concentrated in the minority class: on ViHSD, HATE precision rises (0.5972 → 0.6177) while recall falls slightly (0.6119 → 0.5988), so the model becomes more conservative but more precise — desirable when false accusations of hate are costly. Second, the larger VOZ-HSD gains alongside an accuracy drop indicate that the model trades majority-class accuracy for minority-class recall/precision, exactly the trade-off practitioners want when the minority class is the target of interest. Third, the dependence on cue coverage suggests a clear and inexpensive path to further gains: expanding and refining the cue banks should yield additional improvement without architectural change.
...
ViAmpleHate generalizes AmpleHate for Vietnamese hate speech detection through five adaptations — Vietnamese target extraction, separate target/attack cue channels, relation-bank attention, a corrected batched attention computation, and adaptive relation injection — trained with weighted cross-entropy plus a contrastive loss. On ViHSD and VOZ-HSD in the binary setting it improves macro-F1 and HATE-F1 over the AmpleHate baseline and over TF-IDF, BiLSTM, and PhoBERT-CNN baselines, with the largest gains on the minority class. Our analysis ties these gains to target coverage, which the Vietnamese cue banks raise by more than two orders of magnitude over English NER. The broader lesson is that target-awareness is a useful inductive bias only when adapted to the target language's surface forms, where hate targets are often informal group references rather than named entities.
...
## Appendix B — Cue bank examples

> Representative entries only; the full banks are released with the code. Target cues are referential (not hateful by themselves); attack cues are hostile predicates.

- **Target cues (illustrative):** informal group/person references, regional and demographic labels, group pronouns.
- **Attack cues (illustrative):** predicates expressing contempt, dehumanization, or parasitism (e.g., `khinh`, `ăn_bám`).
